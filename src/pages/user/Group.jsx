...
import { FileText, Loader2, Plus, MoreVertical, ExternalLink, Trash2 } from 'lucide-react'
import Api from '../../api/api'
import { DOCUMENT_DELETED_EVENT, GROUP_OPEN_CREATE_DOC_MODAL } from './DefaultLayoutWithSidebar'
...
export function Group() {
  ...
  useEffect(() => {
    const handler = () => handleCreateModalOpen()
    window.addEventListener(GROUP_OPEN_CREATE_DOC_MODAL, handler)
    return () => window.removeEventListener(GROUP_OPEN_CREATE_DOC_MODAL, handler)
  ...
  function handleCreateSubmit(e) {
    ...
    setIsCreating(true)
    Api.post('/documents', { data: { title, group_id: id } })
      .then(() => {
    ...
    <div className="px-4 py-6 w-full min-h-full overflow-y-auto">
        ...
        {documents.length === 0 ? (
          <div className="min-h-[60vh] flex items-center justify-center">
            <div className="flex flex-col items-center gap-4 animate-[fadeIn_0.4s_ease-out_both]">
              ...
              <p className="text-slate-500 text-base">No documents in this group.</p>
              <button
                type="button"
            ...
            {documents.map((doc) => (
              <div key={doc.id} className="relative flex flex-col gap-2 group text-center">
                ...
                <Link to={`/documents/p/${doc.id}`} className="flex flex-col gap-0.5 -mt-1">
                  <span className="font-semibold text-sm text-slate-800 truncate" title={doc.title || 'Untitled'}>
      ...
      >
        <form onSubmit={handleCreateSubmit} className="py-6 px-6 space-y-4">
          <div>
            <label htmlFor="group-create-doc-title" className="block text-sm font-medium text-slate-700 mb-1">
              Title
            </label>
            <input
              id="group-create-doc-title"
              type="text"
              value={createTitle}
